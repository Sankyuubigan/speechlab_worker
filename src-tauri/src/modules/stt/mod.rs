...
impl SttEngine {
    ...
    pub async fn ensure(
        ...
        let engine_exe = if settings.engine_exe.is_empty() {
            crate::modules::tts::download::resolve_engine_exe(
                &crate::modules::tts::settings::load(app).engine_dir,
                "cpu",
            )
            .to_string_lossy()
            .to_string()
        ...
        *self.status.lock().unwrap() = SttStatus::Starting;
        tray::emit_status(app, &self.status.lock().unwrap());
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            std::thread::spawn(move || {
                ...
                for line in reader.lines().flatten() {
                    let clean = crate::modules::tts::sanitize_crispasr_line(&line);
                    crate::modules::log::app_log(&app_clone, &format!("[stt] {clean}"));
